...
Taken pictures are stored in the folder defined in the configuration key ``[GENERAL][directory]``.
They are named **YYYY-mm-dd-hh-mm-ss_pibooth.jpg** which is the time when first capture of the sequence
was taken. A subfolder **raw/YYYY-mm-dd-hh-mm-ss** is created to store the single raw captures.
...
Extra functionalities can be added using `custom plugins <https://github.com/werdeil/pibooth/blob/master/docs/plugins.rst>`_.
